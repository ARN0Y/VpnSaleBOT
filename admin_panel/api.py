...
async def _pg_settings(database) -> dict[str, str]:
    ...
    return {k: str(rows.get(k, "")) for k in (
        ...
        "pg_group", "pg_verify_tls", "pg_default_days",
    )}


async def _pg_client(database):
...
def _gen_admin_password() -> str:
    """PasarGuard-policy-valid password: 13 chars, 3 uppercase, digits + symbol."""
    pool = (
...
def _slim_admin(a: dict) -> dict:
    ...
    return {
        ...
        "role_name": role.get("name"), "is_owner": bool(role.get("is_owner")),
        "telegram_id": a.get("telegram_id"), "note": a.get("note"),
    }
...
def _slim_pg_user(u: dict) -> dict:
    return {
        ...
        "used_traffic": int(u.get("used_traffic") or 0), "data_limit": u.get("data_limit"),
        "expire": u.get("expire"), "online_at": u.get("online_at"),
        "created_at": u.get("created_at"), "subscription_url": u.get("subscription_url"),
    }
...
async def pg_list_admins(request: Request):
    """Reseller admin roster (excludes owner + the bot's own account), each with
    their total allocated volume (Σ data_limit), computed concurrently."""
    database = db(request)
...
async def pg_admin_users(request: Request, username: str, offset: int = 0, limit: int = 25, search: str = ""):
    ...
    return {
        ...
        "total": int(page["total"]), "offset": offset, "limit": limit,
    }
...
async def pg_list_roles(request: Request):
    ...
    try:
        ...
        await client.close()
    return {"ok": True, "roles": [{"id": r.get("id"), "name": r.get("name"), "is_owner": bool(r.get("is_owner"))} for r in roles]}
...
async def pg_create_admin(request: Request):
    ...
    username = "".join(c for c in str(body.get("username") or "").strip() if c.isalnum() or c == "_")
    if len(username) < 3:
        await client.close()
    ...
    try:
        ...
        await client.close()
    return {"ok": True, "username": username, "password": password, "role_id": role_id,
...
async def pg_create_admin_for_reseller(request: Request, user_id: int):
    """One-click: issue a personal PasarGuard admin for a reseller (creds from
    Telegram id). Stores the username so the UI shows it was created."""
    database = db(request)
    client = await _pg_client(database)
    if client is None:
        return JSONResponse({"ok": False, "error": "پنل پاسارگارد پیکربندی نشده است."}, status_code=400)
