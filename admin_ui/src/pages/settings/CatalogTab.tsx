...
function newPlan(categoryId: string, sort: number): Plan {
  return {
    ...
    volume: { mode: "fixed", gb: 0, days: 30, min_gb: 0, max_gb: 0, step_gb: 0, custom: false },
...
function PlanEditor({
  plan, groups, panels, problems, onChange, onRemove, onDuplicate, onMove, categories,
}: {
  plan: Plan;
            ...
            {variable ? (
              <div className="space-y-3">
                ...
                <label className="flex items-start gap-2 rounded-xl border border-border bg-white/[0.02] p-3 text-sm">
                  <input
                    type="checkbox"
                    checked={Boolean(plan.volume.custom)}
                    onChange={(e) => setVolume({ custom: e.target.checked })}
                    className="mt-0.5 h-4 w-4 accent-[hsl(var(--brand))]"
                  />
                  <span>
                    <span className="font-bold text-white">دکمه‌ی «حجم دلخواه» هم نمایش داده شود</span>
                    <span className="block text-[11px] leading-5 text-muted-foreground">
                      کاربر می‌تواند عدد دقیق حجم را بنویسد (بین «از» و «تا»). برای این کار حتماً باید «تا (گیگ)» پر باشد،
                      وگرنه هر عددی قابل سفارش می‌شود.
                    </span>
                  </span>
                </label>
              </div>
