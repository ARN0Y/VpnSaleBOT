...
import type {
  CatalogBundle, Category, Plan, PlanPricing, PlanTarget, PricingMode, VolumeMode,
} from "@/lib/types";
                ...
                set({
                  target:
                    kind === "pasarguard"
                      ? { kind, group: "" }
                      : kind === "athena"
                        ? { kind, node_id: 0, outbound: "" }
                        : { kind, panel: "1" },
                } as { target: PlanTarget })
              }
              options={[
                { v: "pasarguard" as const, label: "PasarGuard" },
                { v: "xui" as const, label: "3x-ui" },
                { v: "athena" as const, label: athenaLabel || "L2TP" },
              ]}
            />
            {plan.target.kind === "pasarguard" ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="سرور (node)" hint="خالی = سرور پیش‌فرض پنل.">
                  <select
                    value={String((plan.target as { node_id: number }).node_id || 0)}
                    onChange={(e) => set({ target: { kind: "athena", node_id: Number(e.target.value) || 0, outbound: (plan.target as { outbound: string }).outbound } })}
                    className="h-10 w-full rounded-xl border border-input bg-card px-3 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
                    <option value="0">پیش‌فرض پنل</option>
                    {athena.nodes.map((n) => <option key={n.id} value={n.id}>{n.name}</option>)}
                  </select>
                </Field>
                <Field label="خروجی (outbound)" hint="محل خروج ترافیک. خالی = پیش‌فرض پنل.">
                  <select
                    value={(plan.target as { outbound: string }).outbound || ""}
                    onChange={(e) => set({ target: { kind: "athena", node_id: (plan.target as { node_id: number }).node_id, outbound: e.target.value } })}
                    className="h-10 w-full rounded-xl border border-input bg-card px-3 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
                    <option value="">پیش‌فرض پنل</option>
                    {athena.outbounds.map((o) => (
                      <option key={o.name} value={o.name}>{o.label}{o.status && o.status !== "up" ? ` (${o.status})` : ""}</option>
                    ))}
                  </select>
                </Field>
                {athena.error && (
                  <p className="sm:col-span-2 text-[11px] text-amber-300">
                    اطلاعات پنل L2TP خوانده نشد: {athena.error}
                  </p>
                )}
              </div>
            ) : (
...
export function CatalogTab() {
  ...
  const athena = data.athena || { enabled: false, label: "", error: "", nodes: [], outbounds: [] };
  const athenaLabel = athena.label || "سرور L2TP";
