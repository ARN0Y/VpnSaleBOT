...
class ProvisioningService:
    ...
    async def process_pg_plan_renewal(
        self,
        *,
        pg_client,
        user_id: int,
        ...
        its sub link; it receives the package's full volume (fresh — added on top
        of current usage, so exactly the plan's allowance is usable from now) and
        a fresh on_hold validity window. Charged at the package's audience-correct
        price, wallet-only, fully refunded on any failure."""
        clean_sub_id = str(sub_id or "").strip()
        if not clean_sub_id:
            raise ValueError("اشتراک انتخاب‌شده معتبر نیست.")
        kind = "unlimited" if str(pkg.get("kind")) == "unlimited" else "volume"
        cap_gb = max(0, int(pkg.get("gb") or 0))
        pkg_days = max(0, int(pkg.get("days") or 0))
        if cap_gb <= 0:
            raise ValueError("این بسته نامعتبر است.")
        ...
        try:
            ...
            # Fresh plan volume: full package allowance usable from now (added on
            # top of current usage — no separate reset call needed).
            fields: dict = {"data_limit": used + gb_to_bytes(cap_gb)}
            if pkg_days > 0:
                fields["status"] = "on_hold"
                fields["on_hold_expire_duration"] = pkg_days * 86400
            ...
            await self.db.execute(
                "UPDATE subscriptions SET gb=?, is_infinite=?, renewed_count=renewed_count+1, last_renewed_at=? WHERE sub_id=?",
                (int(cap_gb), 1 if kind == "unlimited" else 0, now_ts(), clean_sub_id),
            )
            approved = await self.db.approve_order(order_id)
            if not approved:
                raise RuntimeError("pg plan renewal approval failed after panel update")
            await self.db.execute("UPDATE idempotency_keys SET status='approved' WHERE key=?", (idem,))
            return sub_url
        except Exception:
            if final_total > 0:
    ...
    async def process_reseller_panel_purchase(
        self,
        *,
        pg_client,
        user_id: int,
        ...
        """Sell one reseller panel: an admin account inside PasarGuard.

        Money first, account second, and the account is deleted again if
        anything after it fails — a buyer must never be left paying for an
        account they were never told the password to.

        Returns what the buyer needs to sign in. The password is generated by
        the caller and returned here because the panel will not give it back:
        this is the only moment it can be delivered.
        """
        from . import reseller as reseller_mod

        problems = reseller_mod.validate_package(package)
        if problems:
            raise ValueError(problems[0])

        base_total = int(package["price"])
        traffic = reseller_mod.traffic_bytes(package)
        days = max(0, int(package.get("days") or 0))
        panel_id = f"rp-{user_id}-{now_ts()}-{secrets.token_hex(3)}"
        order_id = f"{user_id}-rpanel-{now_ts()}-{secrets.token_hex(3)}"
        idem = idempotency_key or order_id
        ...
        async with self.db.transaction() as conn:
            ...
            final_total = max(0, base_total - int(discount))
            payment_method = await self._reserve_wallet_payment(
                conn, user_id=user_id, amount_toman=final_total,
                agent_row=agent_row, allow_free=True,
            )
            await conn.execute(
                """
                INSERT INTO orders
                ...
                """,
                (
                    order_id, int(user_id), 0, int(package.get("traffic_gb") or 0), 1,
                    int(base_total), int(base_total),
                    (discount_code or None) if discount else None, int(discount),
                    int(final_total), now_ts(), payment_method.value,
                    "reseller_panel", None, str(username)[:64],
                ),
            )
            await self.db.record_reseller_panel(
                conn,
                panel_id=panel_id, user_id=user_id, pg_username=username,
                package_id=str(package.get("id") or ""), title=str(package.get("title") or ""),
                traffic_bytes=traffic, price_toman=final_total, order_id=order_id,
        ...
        created = False
        try:
            await pg_client.create_admin(
                username=username,
                password=password,
                role_id=int(role_id),
                data_limit=traffic or None,
                note=f"tg:{int(user_id)} · {package.get('title') or 'reseller'}",
                telegram_id=int(user_id),
            )
            created = True
            approved = await self.db.approve_order(order_id)
            if not approved:
                raise RuntimeError("reseller panel order approval failed after creation")
            await self.db.execute(
            ...
            return {
                ...
                "traffic_gb": int(package.get("traffic_gb") or 0),
                "expires_at": expires_at,
                "price": final_total,
            ...
            if created:
                # The account exists but the buyer was never handed it. Leaving
                # it behind would also block the username for good, since panel
                # usernames are unique.
                try:
                    await pg_client.delete_admin(username)
                except Exception:
                    LOG.exception("reseller rollback: could not delete panel admin %s", username)
            await self.db.forget_reseller_panel(panel_id)
            if final_total > 0:
                await self.db.credit_wallet(user_id, final_total)
            await self.db.release_discount(order_id)
            await self.db.reject_order(order_id)
    ...
    async def process_reseller_topup(
        self,
        *,
        pg_client,
        user_id: int,
        ...
        """Add traffic to a panel the buyer already owns.

        The panel's allowance is a total, not a remainder, so topping up means
        raising it by the package size. The new total is computed from what the
        PANEL currently reports rather than from our own row: if an admin edited
        the allowance by hand, the buyer should get their purchase on top of
        ...
        if problems:
            raise ValueError(problems[0])
        username = str(panel.get("pg_username") or "")
        if not username:
            raise ValueError("این پنل معتبر نیست.")

        current = await pg_client.get_admin(username)
        if current is None:
            raise ValueError("این پنل در سرور پیدا نشد. با پشتیبانی تماس بگیرید.")
        existing_limit = max(0, int(current.get("data_limit") or 0))
        extra = reseller_mod.traffic_bytes(package)
        base_total = int(package["price"])
        order_id = f"{user_id}-rtop-{now_ts()}-{secrets.token_hex(3)}"
        idem = idempotency_key or order_id
        ...
        async with self.db.transaction() as conn:
            ...
            payment_method = await self._reserve_wallet_payment(
                conn, user_id=user_id, amount_toman=base_total, agent_row=agent_row,
            )
            await conn.execute(
                """
                INSERT INTO orders
                ...
                (
                    order_id, int(user_id), 0, int(package.get("traffic_gb") or 0), 1,
                    int(base_total), int(base_total), 0, int(base_total), now_ts(),
                    payment_method.value, "reseller_topup", None, username[:64],
                ),
            )
            await self.db.add_reseller_traffic(conn, str(panel["panel_id"]), extra)
            await conn.execute(
                "INSERT INTO idempotency_keys(key,user_id,order_id,status,created_at) VALUES(?,?,?,?,?)",
                (idem, int(user_id), order_id, "reserved", now_ts()),
            )

        new_limit = existing_limit + extra
        try:
            await pg_client.modify_admin(username, data_limit=int(new_limit))
            approved = await self.db.approve_order(order_id)
            if not approved:
                raise RuntimeError("reseller top-up approval failed after the panel was raised")
            await self.db.execute(
                "UPDATE idempotency_keys SET status='approved' WHERE key=?", (idem,)
            )
            await self.db.sync_reseller_panel_usage(
                str(panel["panel_id"]),
                used_bytes=int(current.get("used_traffic") or panel.get("used_bytes") or 0),
                traffic_bytes=new_limit,
            )
            return {
                "panel_id": str(panel["panel_id"]),
                "username": username,
                "added_gb": int(package.get("traffic_gb") or 0),
                "total_bytes": new_limit,
                "price": base_total,
            }
        except Exception:
            with contextlib.suppress(Exception):
                await pg_client.modify_admin(username, data_limit=int(existing_limit))
            async with self.db.transaction() as conn:
                await self.db.add_reseller_traffic(conn, str(panel["panel_id"]), -extra)
            await self.db.credit_wallet(user_id, base_total)
            await self.db.reject_order(order_id)
            await self.db.execute("UPDATE idempotency_keys SET status='failed' WHERE key=?", (idem,))
            raise
