...
from .models import ATHENA_INBOUND_SENTINEL, PG_INBOUND_SENTINEL, PaymentMethod, PanelClientPayload
...
class ProvisioningService:
    ...
    async def process_catalog_purchase(
        ...
        *,
        ...
        try:
            if kind == catalog_mod.TARGET_ATHENA:
                if athena_client is None:
                    raise ValueError("اتصال به سرور برقرار نشد. لطفاً با پشتیبانی تماس بگیرید.")
                base = "".join(c for c in sanitize_client_name(client_name) if c.isalnum() or c == "_")
                base = base[:18].strip("_") or f"u{int(user_id)}"
                node_id = int(target.get("node_id") or 0)
                outbound = str(target.get("outbound") or "")
                for _ in range(requested_qty):
                    username = f"{base}_{secrets.token_hex(4)}"
                    account = await athena_client.create_user(
                        username=username,
                        limit_gb=real_gb,
                        duration_days=days,
                        node_id=node_id or None,
                        outbound=outbound,
                        note=f"telegram:{int(user_id)}",
                    )
                    ...
                    rows.append(
                        PanelClientPayload(
                            user_id=int(user_id),
                            sub_id=username,
                            sub_link=str((account or {}).get("subscription_url") or ""),
                            inbound_id=ATHENA_INBOUND_SENTINEL,
                            client_uuid="",
            ...
            for name in created:
                try:
                    if kind == catalog_mod.TARGET_ATHENA and athena_client is not None:
                        await athena_client.delete_user(name)
                    elif kind == catalog_mod.TARGET_PASARGUARD and pg_client is not None:
