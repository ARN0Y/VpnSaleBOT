...
TARGET_XUI = "xui"
TARGET_KINDS = (TARGET_PASARGUARD, TARGET_XUI)
...
def parse_plan(raw: Any) -> dict | None:
    ...
    return {
        ...
        "volume": {
            ...
            # Buttons cover the common sizes; `custom` is what lets a buyer ask
            # for a volume in between, the way a per-GB shop always could.
            "custom": _bool(volume.get("custom"), False),
        },
        "display": {
...
def allows_custom_gb(plan: dict) -> bool:
    """May the buyer type an exact volume for this plan?"""
    volume = plan.get("volume") or {}
    if _str(volume.get("mode"), VOLUME_FIXED) != VOLUME_VARIABLE:
        return False
    return bool(volume.get("custom")) and custom_gb_range(plan)[1] > 0


def custom_gb_range(plan: dict) -> tuple[int, int]:
    """Inclusive (min, max) a typed volume must fall inside.

    The ceiling is the plan's own max_gb. Without one there is no ceiling to
    enforce, and a plan that lets anyone order a million gigabytes is a bug, so
    the range comes back empty and the custom option stays off.
    """
    volume = plan.get("volume") or {}
    step = _int(volume.get("step_gb"), 0, minimum=0)
    low = _int(volume.get("min_gb"), 0, minimum=0) or step or 1
    high = _int(volume.get("max_gb"), 0, minimum=0)
...
def custom_gb_is_valid(plan: dict, gb: int) -> bool:
    low, high = custom_gb_range(plan)
    if not allows_custom_gb(plan):
        return False
    return low <= int(gb) <= high
...
def validate_plan(plan: dict) -> list[str]:
    ...
    if vmode == VOLUME_VARIABLE:
        ...
        if bool(volume.get("custom")) and custom_gb_range(plan)[1] <= 0:
            problems.append("برای «حجم دلخواه» باید حداکثر حجم مشخص باشد؛ وگرنه هر عددی قابل سفارش است.")
...
def migrate_per_gb_shop(
    *,
    ...
    """Build the first catalog from the per-GB shop that existed before plans.

    This shop never had package lists — it sold volume by the gigabyte plus one
    ...
    # How long a purchase stays valid. On PasarGuard the shop's own
    # `pg_default_days` is what the bot actually applies, and it wins over an
    # unset `purchase_duration_days`; reading only the latter would silently
    # turn 30-day services into ones that never expire.
    days = _int(purchase_duration_days, 0, minimum=0)
    if on_pg and _int(pg_default_days, 0, minimum=0) > 0:
        days = _int(pg_default_days, 0, minimum=0)
    target = (
    ...
    plans = [{
        ...
        "volume": {
            ...
            "days": days,
            "min_gb": step,
            # The old shop let a buyer type any volume up to this ceiling
            # (handlers.max_custom_gb). Keeping both the ceiling and the typed
            # option means nobody loses a size they used to be able to order.
            "max_gb": max(100, step * 4),
            "step_gb": step,
            "custom": True,
        },
...
async def load_catalog(db) -> dict:
    ...
    catalog = migrate_per_gb_shop(
        ...
        pg_default_days=await _i("pg_default_days", "0"),
